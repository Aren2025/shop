...
# ----------------- Stripe test key -----------------
stripe.api_key = "sk_test_51S7qpF67bu9EcDwMJIanBMGmfhh7ouwZj0147EsImXBp7oDtrzWTmdNA1y6KvX8Di1rVgxMb4yqUVuPW9nbXKTAl00rB0oPqp2"
...
def recargar_tarjeta():
    ...
    session_stripe = stripe.checkout.Session.create(
        ...
        mode='payment',
        success_url=url_for('saldo_success', _external=True)+"?coins="+str(PAQUETES[paquete]["coins"])+"&moneda="+moneda,
...
def saldo_success():
    ...
    moneda = request.args.get("moneda", user["moneda"])


    db = get_db()
    user = db.execute("SELECT * FROM users WHERE id=?",(session["user_id"],)).fetchone()
    nuevo_saldo = user["saldo"] + coins
    db.execute("UPDATE users SET saldo=?, moneda=? WHERE id=?",
              (nuevo_saldo, moneda, session["user_id"]))

    db.commit()
    flash(f"Recarga exitosa: {coins} coins ({moneda})","success")
    return redirect(url_for("saldo"))
...
def comprar(producto):
    ...
    if moneda == "MXN":
        precio_local = precio_usd  # usar directamente el precio
    else:
        # convertir a la moneda correspondiente
    ...
    # Datos adicionales
    perfil = cuenta["perfil"] if "perfil" in cuenta.keys() else "P1"
    pin = cuenta["pin"] if "pin" in cuenta.keys() else "No tiene"
    instrucciones = cuenta["instrucciones"] if "instrucciones" in cuenta.keys() else "No modificar Datos"
    renovable = cuenta["renovable"] if "renovable" in cuenta.keys() else "Producto No Renovable"
    dispositivos = cuenta["dispositivo_limit"] if "dispositivo_limit" in cuenta.keys() else "Solo 1 dispositivo"
...
def admin_agregar_cuenta():
    ...
    renovable = request.form.get("renovable", "")
    dispositivo_limit = request.form.get("dispositivo_limit", "")
...
def admin_precios():
    ...
    # Convertimos precios actuales a MXN (si usas otra moneda se podría agregar)
    ...
    if request.method == "POST":
        for producto in PRODUCTOS.keys():
            ...
            if nuevo_precio:
                try:
                    # Guardamos directamente el valor ingresado como precio en MXN
